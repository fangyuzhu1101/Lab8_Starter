describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    for (let i = 0; i < prodItems.length; i++) {
      ...
      dataProperty = await prodItems[i].getProperty('data');
      // Convert that property to JSON
      plainValue = await dataProperty.jsonValue();
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const countTextProperty = await cartCount.getProperty('innerText');
    const cartCountValue = await countTextProperty.jsonValue();
